...
- **CodeQL**: configured via `security.yml` (`github/codeql-action` init/autobuild/analyze for `javascript`), runs on push/PR to `main` and weekly (`30 4 * * 1`). Requires GitHub "Code scanning" to be enabled for the repo (Settings → Code security) — this is a repo-setting toggle that cannot be verified or changed from the local filesystem; flagged for the user to confirm in the GitHub UI.
